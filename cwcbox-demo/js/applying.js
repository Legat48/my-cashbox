...
interfaceApp.scrollFuncVert('.receipt', 9); // document.querySelector('.category__wrap').addEventListener('mousedown', () => {
//   alert('test')
// })
...
var xDown = null;
var yDown = null;
var testprevBtn = document.querySelector('.category__btn_prev');
var testnextBtn = document.querySelector('.category__btn_next');
...
function QQhandleTouchStart(evt) {
  var firstTouch = QQgetTouches(evt);
...
function QQhandleTouchMove(evt) {
  ...
  if (Math.abs(xDiff) > 50 || Math.abs(yDiff) > 50) {
    if (Math.abs(xDiff) > Math.abs(yDiff)) {
      xDown = null;
      yDown = null; // alert('test'+`${xDown} - ${xUp}/ ${yDown} - ${yUp}`)
    ...
    if (Math.abs(xDiff) < Math.abs(yDiff)) {
      xDown = null;
      yDown = null; // alert('test'+`${xDiff} - ${xUp}/ ${yDiff} - ${yUp}`)
...
function handleTouchMove(evt) {
  ...
  if (Math.abs(xDiff) > 50 || Math.abs(yDiff) > 50) {
    if (Math.abs(xDiff) > Math.abs(yDiff)) {
      ...
      xDiff > 0 ? alert('Left' + "".concat(xDiff, " ").concat(yDiff)) : alert('Right' + "".concat(xDiff, " ").concat(yDiff));
    }
    ...
    if (Math.abs(xDiff) < Math.abs(yDiff)) {
      ...
      yDiff > 0 ? alert('Up' + "".concat(xDiff, " ").concat(yDiff)) : alert('Down' + "".concat(xDiff, " ").concat(yDiff));
    }
  }
}
