...
function ProductOrder(technical_card, accumClientPoints, count, price, total, discount, points) {
  var weighted = arguments.length > 7 && arguments[7] !== undefined ? arguments[7] : false;
...
function Transaction(order) {
  ...
  this.uniqid = "".concat(Number(order.id)).concat(order.hashcode); // уникальный айди
  ...
  var productArr = []; // создаем массив товаров чека

  try {
    q = "SELECT id, idProd, idOrder, name, productCategory, bulkvalue, bulkuntils, price, sale, debitClientBonus, saleMaxPrice, interimPrice, offreckoningItem, cashbackPercent, amount, priceFinal, cashback, cashbackSum, productSum FROM orderItem WHERE idOrder = ".concat(Number(order.id));
    ans = db.query(q);
    ...
    productTableArr.forEach(function (e) {
      var newProduct = new Product(e.id, e.idProd, e.idOrder, e.name, e.productCategory, e.bulkvalue, e.bulkuntils, e.price, e.sale, e.debitClientBonus, e.saleMaxPrice, e.interimPrice, e.offreckoningItem, e.cashbackPercent, e.amount, e.priceFinal, e.cashback, e.cashbackSum, e.productSum); //напихали товары в массив
  ...
  productArr.forEach(function (e) {
    if (e.productCategory == -1) {
      var promo = new PromoOrder(e.idProd, order.accumClientPoints, e.amount, e.price, e.priceFinal, e.sale, e.debitClientBonus);

      _this.promotions.push(promo);
    } else {
      var product = new ProductOrder(e.idProd, order.accumClientPoints, e.amount, e.price, e.priceFinal, e.sale, e.debitClientBonus);
...
function LoyaltyRequestProduct(title, quantity, price) {
  ...
  this.quantity = "".concat(quantity); //кол-во

  this.price = "".concat(price); // финальная цена
...
function LoyaltyRequest(order) {
  ...
  this.uniq_id = "".concat(Number(order.id)).concat(order.hashcode); //уникальное айди чека

  this.userPhone = "".concat(order.idClient); //номер клиенте

  this.country = subdomain; //страна

  this.amount = "".concat(order.orderSum); // стоимость чека без баллов и скидок

  this.total_amount = "".concat(order.orderSumFinal); // финальная стоимость чека

  this.discount = '0'; //всегда 0 кидаем по скидкам

  this.bonuses = '0'; // считаем и отправляем разницу в баллах до и после операции

  if (order.accumClientPoints == '1' || order.accumClientPoints == '2') {
    this.bonuses = "".concat(Number(order.clientPoints) - Number(order.initiallyPoints));
  }

  this.date = "".concat(order.unix); // таймштамп
  // this.point = '13'; //ВРЕМЕНННО

  this.point = "".concat(pointCashbox); //номер точки

  this.positions = []; //товары
  // берем товары по чеку

  try {
    var q = "SELECT id, idProd, idOrder, name, bulkvalue, bulkuntils, price, sale, debitClientBonus, saleMaxPrice, interimPrice, offreckoningItem, cashbackPercent, amount, priceFinal, cashback, cashbackSum, productSum FROM orderItem WHERE idOrder = ".concat(Number(order.id));
    var ans = db.query(q);
    var productTableArr = [];
    ...
    productTableArr.forEach(function (e) {
      var newProduct = new LoyaltyRequestProduct(e.name, e.amount, e.priceFinal); //напихали товары в массив

      _this2.positions.push(newProduct);
    });
  } catch (e) {
    try {
      db.conn.close();
