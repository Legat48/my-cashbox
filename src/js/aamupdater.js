...
function updater()
{
    ...
    pointCashbox = point();
		...
		for(var i in iFiv)
		{
			if(typeof(i)!='undefined'){
				...
				var itshave = this.haveindb(fName, ver);
				//this.addlineinlog('Есть файл: '+itshave);
		...
		for(var i in iFiv)
		{
			if(typeof(i)!='undefined'){
				...
				if(itshave != 1){
					...
					try{this.backupfile(fName);}catch(e){logger(e.message);}

					var fdurl = "https://bridge.cwsystem.ru/updatekassa/backend/download?point="+pointCashbox+"&file="+fName+"&key=nokey";
					//this.addlineinlog(fdata);
	...
	this.filesave = function(fName, fData, ver)
	{
		try{
			...
			fileExt = fileExt == 'js' || fileExt == 'css' ? fileExt : '';  // кидает все js и css в папки
			const pathTo = '.\\update\\'+fileExt+'\\'+fName;
			...
			var http = new ActiveXObject("MSXML2.XMLHTTP");
			  ...
			  try {
				const resp = JSON.parse(http.responseText).type;
				if(resp == 'error') {
					this.addindb(fName,ver);
				...
				progress += 1
				let procent = Math.ceil(progress/toLoad*100)
				this.addlineinlog('filesave: '+fName+': '+procent+'%');
			  }

			  ado.Write(http.responseBody);
			  ado.SaveToFile(pathTo, 2);
			  //ado.close();
			return 1;
		}catch(e){
  ...
  var strFile = '';
	...
	this.addindb = function(filename, vers)
	{
		try{
			...
			//this.addlineinlog('4');
			let obj = {};//{name:filename,version:vers};
			obj[filename] = vers;//{name:filename,version:vers};
			jsonF.push(obj);
			//this.addlineinlog(JSON.stringify(jsonF));
			this.savejsonfile(JSON.stringify(jsonF));
			return true;
		}catch(e){
	...
	this.loadjsonfile = function()
	{
		...
		if( !iStream ) { alert( "Can't open file"+".\\rev\\rev-manifest.json" ); this.makerev(); return "[]"; }
	...
	{
		if(typeof(myLog)!='undefined'){myLog.innerHTML += msg+"<br>";}
	}
}
var upd = null;
...
{
	try{
		if (typeof(upd) == 'undefined'){upd = new updater();}
		if (upd == null) {upd = new updater();}
		upd.init();
	}catch(e) {
    // logger(e.message+" "+e.line+" "+JSON.stringify(e));
  }
...
function reqxmlA(dir, param)
{
  pointCashbox = point();
	...
	var reslt = "";
	URL = escapeHtml(escapeHtml("http://bridge.cwsystem.ru/updatekassa/backend/"+dir+param))
	//"http://updatekassa/backend/check?point="+pointCashbox+"&key=LBcBBN"
